...
const check = response => {
  ...
  if (response.data) {
    if (response.data.error) {
      throw new Error(
        `LinkedIn API responded with a failure: ${response.data.error}, ${response.data.error_description}`
      );
  ...
  throw new Error(
    `LinkedIn API responded with a failure: ${response.status} (${response.statusText})`
  );
};
...
module.exports = (apiBaseUrl, loginBaseUrl) => {
  ...
  return {
    ...
    getToken: (code, state) => {
      ...
      return axios
        .post(urls.oauthToken, qs.stringify(data), {
          headers: {
